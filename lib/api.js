...
const API = function ({blueprint, targetDirectory}) {
  ...
  this.walk = io.walkDirectory(this.baseDirectory).then(({paths, tree}) => {
    this.paths = paths
    this.tree = tree
  }).then(() => {
...
API.prototype._processLevel = function ({
  ...
  const subLevelNamePlural = subLevelName && (pluralise ? pluralize(subLevelName) : subLevelName)
  ...
  if (!tree) {
    ...
    const read = io.readFile(filePath, true).then(contents => {
      Object.assign(item, id, contents)
    })
